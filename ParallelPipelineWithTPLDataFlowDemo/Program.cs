...
namespace ParallelPipelineWithTPLDataFlowDemo
{
    class Program
    {
        ...
        async static Task ProcessAsynchronously()
        {
            ...
            var inputBlock =
                new BufferBlock<int>(new DataflowBlockOptions() {BoundedCapacity = 5, CancellationToken = cts.Token});
            var filter1Block = new TransformBlock<int, decimal>(
                n =>
                {
                    decimal result = Convert.ToDecimal(n*0.97);
                    Console.WriteLine("Filter 1 send {0} to the next stage on thread id {1}",result,Thread.CurrentThread.ManagedThreadId);
                    Thread.Sleep(100);
                    return result;
            ...
            var outputBlock = new ActionBlock<string>(
                s => Console.WriteLine("The final result is {0} on thread id {1}", s,
                    Thread.CurrentThread.ManagedThreadId), new ExecutionDataflowBlockOptions() {MaxDegreeOfParallelism = 4, CancellationToken = cts.Token});
            ...
            try
            {
                Parallel.For(0, 20, new ParallelOptions() {MaxDegreeOfParallelism = 4, CancellationToken = cts.Token},
                    i =>
                    {
                        Console.WriteLine("added {0} to source data on thread id {1}", i,
                            Thread.CurrentThread.ManagedThreadId);
